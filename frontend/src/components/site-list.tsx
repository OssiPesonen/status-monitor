							...
							<Table>
								...
								<TableBody>
									...
									{groupedStatuses.map((site: Site) => {
										return <Row key={site.id} site={site} />;
									})}
								</TableBody>
							</Table>
						</div>
		...
		<TableRow key={site.id}>
			<TableCell>
				<div className="flex items-center gap-2">
					...
					<Link href={`/monitor/${site.id}`} className="text-orange-500">
						{site.name}
					</Link>
				</div>
			</TableCell>
			<TableCell>
				<div className="flex gap-1 m-auto justify-start">
					{site.statuses?.slice(0, 12).map((status, i) => (
						<div className="has-tooltip relative" key={`status-${status.siteId}-${status.time}`}>
							...
							{status.siteStatus === SiteStatus.UP ? (
								<div className="h-8 w-2 rounded-xl bg-green-400 block" />
							) : (
								<div className="h-8 w-2 rounded-xl bg-red-400 block" />
							)}
						</div>
					))}
				</div>
