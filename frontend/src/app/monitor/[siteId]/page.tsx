...
import { type Site, SiteStatus, type StatusPing } from "@/types";
import { Button } from "@/components/ui/button";
import EditSiteDialog from "@/components/edit-site-dialog";
import { siteDataKey } from "@/components/site-list";
...
const Monitor = () => {
	...
	const individualSiteDataKey = `site-data-${params.siteId}`;

	const { isPending, data, refetch } = useQuery<Site>({
		queryKey: [individualSiteDataKey],
		queryFn: () =>
			fetch(`${process.env.NEXT_PUBLIC_API_URL}/site/${params.siteId}`, {
				credentials:
	...
	return (
		<Layout>
			<div className="flex items-start">
				<Breadcrumb className="mb-6 flex-1">
					<BreadcrumbList>
						<BreadcrumbItem>
			...
			{ data && <EditSiteDialog site={data} open={isEditDialogOPen} handleOpenChange={handleOpenChange} invalidateQueryKeys={[individualSiteDataKey, siteDataKey]} /> }
		</Layout>
	);
};
